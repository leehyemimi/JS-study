	...
	t.fn.bxSlider = function (n) {
		...
		if (!t(o).data("bxSlider")) {
			var l = function () {
				...
				s.settings.auto && s.settings.autoStart && (f() > 1 || s.settings.autoSlideForOnePage) && W(),
				s.settings.ticker && O(),
				...
				if (s.settings.moveSlides > 0) {
					...
					t = Math.ceil(s.children.length / m())
				} else
				...
				s.children.length > s.settings.maxSlides && s.active.last && !s.settings.infiniteLoop ? "horizontal" === s.settings.mode ? (t = (e = s.children.last()).position(),
					...
					S(-t.top, "reset", 0)) : (t = s.children.eq(s.active.index * m()).position(),
				...
				s.settings.pagerCustom ? s.pagerEl = t(s.settings.pagerCustom) : (s.pagerEl = t('<div class="bx-pager" />'),
					...
					b()),
					s.pagerEl.on("click touchend mouseover", "a", z),
				1 == s.settings.auto && s.pagerEl.on("click touchend mouseover", function () {
					o.stopAuto()
				}).mouseleave(function () {
					o.startAuto()
				})
				...
				1 == s.settings.auto && (s.controls.next.on("click touchend", function () {
					o.stopAuto()
				}).mouseleave(function () {
					o.startAuto()
				}),
					s.controls.prev.on("click touchend", function () {
						o.stopAuto()
					}).mouseleave(function () {
						o.startAuto()
					}))
				...
				s.pagerEl.find("a").removeClass("active"),
					s.pagerEl.each(function (i, n) {
						if (t(n).find("a").eq(e).addClass("active"),
								s.settings.cutLimit) {
							var o = e - s.settings.cutLimit + 1;
							o < 0 && (o = 0);
							var r = t(n).find("a");
							r.hide(),
								r.slice(o, o + s.settings.cutLimit).show()
						}
					})
			}
				, A = function () {
				if (s.settings.infiniteLoop) {
					var t = "";
					0 === s.active.index ? t = s.children.eq(0).position() : s.active.index === f() - 1 && s.carousel ? t = s.children.eq((f() - 1) * m()).position() : s.active.index === s.children.length - 1 && (t = s.children.eq(s.children.length - 1).position()),
				...
				if (s.controls.el.addClass("disabled"),
					...
					if (s.touch.start.x = i[0].pageX,
							s.touch.start.y = i[0].pageY,
						"function" == typeof PointerEvent && void 0 === e.pointerId)
						return;
					i = void 0 !== e.changedTouches ? e.changedTouches : [e],
			...
			return o.goToSlide = function (e, i) {
				...
				if (s.oldIndex = s.active.index,
					...
					s.settings.controls && D(),
						"fade" === s.settings.mode ? (s.settings.adaptiveHeight && s.viewport.height() !== u() && s.viewport.animate({
							height: u()
							...
							!s.settings.infiniteLoop && s.carousel && s.active.last ? "horizontal" === s.settings.mode ? (g = (p = s.children.eq(s.children.length - 1)).position(),
								c = s.viewport.width() - p.outerWidth()) : (n = s.children.length - s.settings.minSlides,
								g = s.children.eq(n).position()) : s.carousel && s.active.last && "prev" === i ? (r = 1 === s.settings.moveSlides ? s.settings.maxSlides - m() : (f() - 1) * m() - (s.children.length - s.settings.maxSlides),
								g = (p = o.children(".bx-clone").eq(r)).position()) : "next" === i && 0 === s.active.index ? (g = o.find("> .bx-clone").eq(s.settings.maxSlides).position(),
								s.active.last = !1) : e >= 0 && (l = e * parseInt(m()),
								g = s.children.eq(l).position()),
						void 0 !== g && (a = "horizontal" === s.settings.mode ? -(g.left - c) : -g.top,
							S(a, "slide", s.settings.speed)),
							s.working = !1),
					s.settings.ariaHidden && j(s.active.index * m())
				}
			}
				,
				...
				o.destroySlider = function () {
					...
					s.settings.keyboardEnabled && t(document).unbind("keydown", X),
						...
						t(window).off("blur", L).off("focus", H))
				}
				,
